...
#%% 
    ...
    start = up_ep['start'].values 
    start = nap.Tsd(start)
    
    ends = up_ep['start'].values + 0.15
    ends = nap.Tsd(ends)
    
    ang_start = start.value_from(wtavg)
    mrl_start = start.value_from(MRL)
    
    ang_ends = ends.value_from(wtavg)
    mrl_ends = ends.value_from(MRL)
    
    angdiff = abs(ang_ends.values - ang_start.values)
    angdiff = np.minimum((2*np.pi - abs(angdiff)), abs(angdiff))
    
    mean_ends_mrl.append(np.mean(mrl_ends))
    mean_start_mrl.append(np.mean(mrl_start))
    
    plt.figure()
    plt.title('MRL Distributions')
    plt.hist(mrl_ends, label = 'end')
    plt.hist(mrl_start, alpha = 0.8, label = 'start')
    plt.xlabel('MRL')
    plt.ylabel('Counts')
...
#%%
    ...
    for i in range(len(mrl_start)):
        if (mrl_start.iloc[i] > threshold) & (mrl_ends.iloc[i] > threshold):
            tokeep.append(i)        
    ...
    ax.set_xscale('log')
    ax.set_yscale('log')
    im = ax.scatter(mrl_start.values[tokeep],mrl_ends.values[tokeep], c = ang_keep, cmap = 'hot',s = 1)
    f.colorbar(im, ax=ax)
    ax.set_xlabel('Start MRL')
    ax.set_ylabel('Ends MRL')
...
plt.figure()
plt.plot(sleep_binwidth,mean_start_mrl,'o-', label = 'start')
plt.plot(sleep_binwidth,mean_ends_mrl, 'o-', label = 'ends')
...
# a = stats.binned_statistic_2d(mrl_start.values[tokeep], mrl_ends.values[tokeep], ang_keep, statistic = 'mean', bins = 20)
