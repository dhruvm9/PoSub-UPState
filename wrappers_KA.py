...
def makePositions(path, file_order, episodes, n_channels,channel_optitrack, names = ['ry', 'rx', 'rz', 'x', 'y', 'z'], update_wake_epoch = True):
    """
    ...
    files = os.listdir(path)    
    for f in file_order:
        if not np.any([str(f)+'.csv' in g for g in files]):
    ...
    for i, f in enumerate(file_order):
        csv_file = os.path.join(path, "".join(s for s in files if str(f)+'.csv' in s))
...
def loadPosition(path, events = None, episodes = None, n_channels = 1,channel_optitrack=1):
    """
    ...
    if not os.path.exists(file):
        print(n_channels)
        makePositions(path, events, episodes, n_channels,channel_optitrack)
...
def loadTTLPulse(file, n_channels, channel, fs = 20000):
    """
    ...
    with open(file, 'rb') as f:
        data = np.fromfile(f, np.uint16).reshape((n_samples, n_channels))[:,channel]
...
def makeHFEpochs(data_directory,name, n_channels=4, fs=20000):
    """
    commmnts
    """
    ...
    channel2=data[:,1].astype(np.int32) #this extracts the second ttl from channel 2
    peaks2, _ = scipy.signal.find_peaks(np.diff(channel2), height = 40000) #height gives the ttl cutoff!
    trough2, _ = scipy.signal.find_peaks(np.diff(channel2)*-1, height = 40000)

    channel3=data[:,2].astype(np.int32) #this extracts the second ttl from channel 3
    peaks3, _ = scipy.signal.find_peaks(np.diff(channel3), height = 40000) #height gives the ttl cutoff!
    trough3, _ = scipy.signal.find_peaks(np.diff(channel3)*-1, height = 40000)
    
    channel4=data[:,3].astype(np.int32) #this extracts the second ttl from channel 4
    peaks4, _ = scipy.signal.find_peaks(np.diff(channel4), height = 40000) #height gives the ttl cutoff!
    trough4, _ = scipy.signal.find_peaks(np.diff(channel4)*-1, height = 40000)
    

    hf_ep=loadEpoch(data_directory,'hf')
   # wake_ep=loadEpoch(data_directory,'wake')
    
    timestep = np.arange(0, n_samples)/fs + hf_ep.as_units('s').loc[2,'start'] # re-run for every 
    #timestep = np.arange(0, n_samples)/fs + wake_ep.as_units('s').loc[0,'start'] # re-run for every 
    ...
    #GREY SCREEN
    ###############
    '''start1_grey=timestep[trough1]
    end1_grey=timestep[peaks1]  #this is the grey screen ep
    
    start2_grey=timestep[trough2]
    end2_grey=timestep[peaks2]
    
    start3_grey=timestep[trough3]
    end3_grey=timestep[peaks3]
    
    start4_grey=timestep[trough4]
    end4_grey=timestep[peaks4]
    
    ##
    start1_grey=start1_grey.reshape(len(start1_grey)//2,2)
    end1_grey=end1_grey.reshape(len(end1_grey)//2,2)
    
    start2_grey=start2_grey.reshape(len(start2_grey)//2,2)
    end2_grey=end2_grey.reshape(len(end2_grey)//2,2)
    
    start3_grey=start3_grey.reshape(len(start3_grey)//2,2)
    end3_grey=end3_grey.reshape(len(end3_grey)//2,2) 
    
    start4_grey=start4_grey.reshape(len(start4_grey)//2,2)
    end4_grey=end4_grey.reshape(len(end4_grey)//2,2)
    
    #Grey Epochs
    grey1_ep=nts.IntervalSet(start1_grey[:,0], end1_grey[:,0],time_units='s') 
    grey2_ep=nts.IntervalSet(start2_grey[:,0], end2_grey[:,0],time_units='s') 
    grey3_ep=nts.IntervalSet(start3_grey[:,0], end3_grey[:,0],time_units='s') 
    grey4_ep=nts.IntervalSet(start4_grey[:,0], end4_grey[:,0],time_units='s') 
    '''
    
    ##############
    #DRIFTING
    ##############
    start1=timestep[peaks1]  #this gives you the start of the ttl 
    end1=timestep[trough1]
    
    start2=timestep[peaks2]
    end2=timestep[trough2]

    start2_grey=timestep[trough2]
    end2_grey=timestep[peaks2]

    start3=timestep[peaks3]
    end3=timestep[trough3]
    
    start4=timestep[peaks4]
    end4=timestep[trough4]
    
    start1=start1.reshape(len(start1)//2,2)
    end1=end1.reshape(len(end1)//2,2)
    
    start2=start2.reshape(len(start2)//2,2)
    end2=end2.reshape(len(end2)//2,2)
    
    start3=start3.reshape(len(start3)//2,2)
    end3=end3.reshape(len(end3)//2,2) 
    
    start4=start4.reshape(len(start4)//2,2)
    end4=end4.reshape(len(end4)//2,2)
    
    ###############
    #STATIC
    ###############
    constant1_ep=nts.IntervalSet(start1[:,0], end1[:,0],time_units='s') 
    constant2_ep=nts.IntervalSet(start2[:,0], end2[:,0],time_units='s') 
    constant3_ep=nts.IntervalSet(start3[:,0], end3[:,0],time_units='s') 
    constant4_ep=nts.IntervalSet(start4[:,0], end4[:,0],time_units='s') 
    
             
    grating_left_ep=nts.IntervalSet(start1[:,1],end1[:,1], time_units='s')
        
    grating_right_ep=nts.IntervalSet(start2[:,1],end2[:,1], time_units='s')
    
    grating_downward_ep=nts.IntervalSet(start3[:,1],end3[:,1], time_units='s')
    
    grating_upward_ep=nts.IntervalSet(start4[:,1],end4[:,1], time_units='s')
    
    output={            
            #'gray_screen_rgt':grey1_ep,
            #'static_right_90': constant2_ep,
            'grating_right': grating_right_ep,
            
            #'gray_screen_lft':grey2_ep,
            #'static_left_90': constant1_ep,
            'grating_left': grating_left_ep,
